...
class SMAlnsOptimizer(BaseOptimizer):
    """Saving Method + ALNS hybrid: SM constructs initial solution, ALNS improves it."""
    ...
    def _alns_improve(self, route: Route) -> Route:
        ...
        for it in range(self.alns_iterations):
            ...
            if self.verbose and (it + 1) % 20 == 0:
                ...
                print(
                    f"[SM+ALNS] iter={it+1:>4}/{self.alns_iterations}"
    ...
    def optimize(self) -> Route:
        ...
        print(f"\n{'='*60}")
        print(f"[SM+ALNS] START  SM construction + ALNS improvement")
        print(f"[SM+ALNS] alns_iterations={self.alns_iterations}  n_remove={self.n_remove}")
        print(f"{'='*60}")

        # Phase 1: Construct initial solution using Saving Method
        sm = SMOptimizer(self.data, self.request, verbose=False)
        initial_route = sm.optimize()
        initial_fitness = self.evaluator.fitness(initial_route)

        if self.verbose:
            ev = self.evaluator.evaluate_route(initial_route)
            print(
                f"[SM+ALNS] SM initial: fitness={initial_fitness:.4f}"
                f"  dist={ev['total_distance_km']:.2f}km"
                f"  time={ev['total_time_min']:.1f}min"
                f"  co2={ev['total_co2_kg']:.3f}kg"
            )
            print(f"[SM+ALNS] Starting ALNS improvement ({self.alns_iterations} iterations)...")
        ...
        print(
            f"[SM+ALNS] DONE  fitness={self.best_fitness:.4f}"
